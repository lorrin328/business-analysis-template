...
- `coverage`：查询主题数、来源数、官方来源数、经主体核验的微信公众号来源数和研究边界；
...
来源计数和页面长度属于可确定计算的发布元数据，由程序按实际证据台账和字段上限校准。独立验证不可达的来源只有在引用项仍有替代证据、宏观/监管仍保留官方 A 级、同业仍保留一手 A/B 级且报告仍不少于 8 项来源时才可剔除；唯一证据或关键等级证据不可降级，必须阻止发布并继续修复。微信公众号来源还必须保留经独立核验的账号主体、标题、正文锚点和内容哈希。
