# 经营分析看板 - 需求与开发文档
## v1.0.109 更新说明（2026-07-28）
**类型**：ui / honor / usability

**变更内容**：
- 星钻联盟页面由九个一级页签收拢为四个业务入口，合并机构与项目分析、人员与预警查询。
- 首屏指标精简为五项核心结果，会员结构、机构会员和本月贡献在总览中集中呈现。
- 管理操作迁移至“数据与规则”，普通查看条件与数据检查、重新测算、导出明细分开。
- 人员追踪增加结果数量和折叠明细，月度预警默认只展示督导所需字段。
- 页面文案改为直接业务表达，版本号和前端缓存参数统一到 `v1.0.109`。
