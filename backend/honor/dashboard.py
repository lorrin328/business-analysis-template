"""Dashboard payload builder for honor alliance results."""
...
def _aggregate_rows(
    ...
    *,
    ...
    for row in summaries:
        ...
        current = current_index.get((str(row.get("staff_code") or ""), str(row.get("business_line") or ""), str(row.get("role_type") or "")))
...
def _policy_qj_index(source_policy: list[dict[str, Any]]) -> dict[tuple[int, str, str], dict[str, float]]:
    ...
    for row in source_policy:
        ...
        item["policy_count"] += float(row.get("is_longterm") or 0)
    return index
...
def _manager_history(
    ...
    person_month_index = {
        (int(row.get("month") or 0), str(row.get("staff_code") or ""), str(row.get("business_line") or ""), str(row.get("role_type") or "")): row
        for row in person_month
    }
    ...
    for manager in source_staff:
        ...
        team_person_rows = [
            person_month_index.get((month, str(staff.get("staff_code") or ""), line, "个人"))
            for staff in team_rows
        ]
        team_person_rows = [row for row in team_person_rows if row]
        manager_person = person_month_index.get((month, str(manager.get("staff_code") or ""), line, role_type), {})
        qj_total = 0.0
        standard_total = 0.0
        for staff in team_rows:
...
def _build_monthly_warnings(
    months: list[dict[str, Any]],
    person_index: dict[tuple[str, str, str], dict[str, Any]],
    exceptions: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    ...
    for row in [item for item in months if int(item.get("month") or 0) == latest_month]:
        staff_key = (str(row.get("staff_code") or ""), str(row.get("business_line") or ""), str(row.get("role_type") or ""))
    ...
    for row in exceptions:
        staff_code = str(row.get("staff_code") or "")
        person = next((p for (code, _line, _role), p in person_index.items() if code == staff_code), {})
        rows.append(
            {
