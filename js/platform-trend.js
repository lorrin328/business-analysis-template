// platform-trend.js — 平台趋势图表 (含 platformMock 数据和渲染逻辑)
(function (window) {    // ---------- Chart 1: Platform Trend ----------
  ...
  "2026": {
    ...
    function trimDailySeries(values) {
      let lastValid = -1;
      ...
      return values.slice(0, lastValid + 1).map(v => Math.round((v || 0) * 10) / 10);
    }
    ...
    function getJingdaiMonthFallback(year, premiumType, month) {
      ...
      return trimDailySeries(monthData[key]);
    }
    ...
    function getMonthDailyCumulative(year, premiumType, selectedKeys, month) {
      ...
      if (useOrgDaily) {
        ...
        if (hasDailyRows) {
          ...
          const result = trimDailySeries(cumulative);
          return result;
        }
      ...
      if (pm && pm.month && pm.month[month] && pm.month[month][typeKey]) {
        ...
        if (combined) {
          const result = trimDailySeries(combined);
          return result;
        }
      }
    ...
    function getPeriodDailyCumulative(year, premiumType, selectedKeys, monthList) {
      ...
      monthList.forEach(month => {
        const monthSeries = getMonthDailyCumulative(year, premiumType, selectedKeys, month);
        monthSeries.forEach((v, idx) => {
  ...
  window.getPlatformOption = getPlatformOption;
  window.loadYearFromApi = loadYearFromApi;
  window.switchYear = switchYear;
  window.refreshPlatformChart = refreshPlatformChart;
  window.selectedYear = selectedYear;`n  window.currentTimeDim = currentTimeDim;`n  window.selectedMonth = selectedMonth;`n  window.selectedQuarter = selectedQuarter;`n  window.selectedSeries = selectedSeries;`n  window.switchTimeDim = switchTimeDim;`n  window.switchSubPeriod = switchSubPeriod;`n  window.switchPremiumType = switchPremiumType;`n  window.toggleSeries = toggleSeries;`n  window.toggleOrg = toggleOrg;`n  window.selectedPlatformOrgs = selectedPlatformOrgs;`n})(window);
