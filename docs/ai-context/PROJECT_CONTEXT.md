...
- 市场研判由独立`market-ai`账号调用Claude Code + DeepSeek V4；Pro负责首次深度研究，Flash负责轻量子任务和第一次定向修复，失败再由Pro升级一次。每天北京时间凌晨1点检查，距上次成功报告满3个自然日时生成结构化报告。发布需同时通过来源独立验证和不低于9.0分的五维质量门槛；FastAPI只读取已发布JSON，不在请求线程中启动模型。
