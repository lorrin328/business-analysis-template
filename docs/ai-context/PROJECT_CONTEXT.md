...
- 市场研判由独立`market-ai`账号调用Claude Code + DeepSeek V4；Pro负责首次深度研究，Flash负责轻量子任务和第一次定向修复，失败再由Pro升级一次。每天北京时间凌晨1点检查，距上次成功报告满3个自然日时生成结构化报告。FastAPI只读取已通过证据校验的JSON，不在请求线程中启动模型。
