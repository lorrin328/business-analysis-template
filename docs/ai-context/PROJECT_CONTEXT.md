...
v1.0.148 已同步GitHub、发布GHCR并部署Ubuntu，最终应用提交 `5490a546cee29e2bbd2e9977b8281816be5a6b60`。证保常规网点表已在“券商人员”后增加按太平人员代码去重的“太平人员”列；本轮不改变接口聚合、数据库结构或生产数据。Windows回归730项通过、1项跳过，36项Node检查通过，GitHub验证和镜像发布成功；可信归档保库部署并经正式NPM公网登录浏览器确认147个网点、表头顺序、实际人数和空控制台，生产数据审计零问题，发布状态accepted。完整证据、恢复点及独立`market-analysis.service`已知异常见 `docs/RELEASE_REVIEW_v1.0.148.md`，其状态优先于历史日志。
