...
## 本地候选：四类日常Excel明细API（2026-09-12，未发布）

已核验四类日常源表全部命名业务列在本地SQLite原始表存在；新增字段目录、全字段分页明细、选列与筛选、独立 `ai_raw_data` 权限及导入变化检查。201项相关测试通过，真实本地库副本200,139行所有字段经API完整读取并与独立SQL一致。最新Excel与本地库行数不同，未自动导入；原始表不等于整本Excel或历次导入归档。本次尚未推送或部署，线上版本说明沿用下文；详见 `docs/EXCEL_API_AUDIT_20260912.md`。

## 当前主应用：v1.0.154（2026-09-11）
