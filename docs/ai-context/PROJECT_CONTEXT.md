...
v1.0.149 正在修复市场研判最终证据对齐后的确定性失败：对齐后出现的未支持数字、日期或政策状态将在同一次任务内追加一次定向修复并重新核验来源；内容门禁最终失败使用不可自动重试的独立退出码，避免systemd重复运行。该修复不放宽9分门槛，不改变经营数据库或指标口径；GitHub、生产部署和完整研究验收状态以本节后续更新及对应发布复核文件为准。
