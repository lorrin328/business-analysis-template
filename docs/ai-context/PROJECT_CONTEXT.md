...
v1.0.149 已修复并上线：最终证据对齐后出现的未支持数字、日期或政策状态会在同一次任务内追加一次定向修复并重新核验来源；内容门禁最终失败使用不可自动重试的独立退出码，避免systemd重复运行。生产已从原`verify`失败断点续跑并发布`market-20260904-150238`，质量评分9.4，含11个模块和26个已核验来源；定时器、手动触发、公网鉴权、数据保护和恢复点均通过验收，发布状态为accepted。该修复不放宽9分门槛，不改变经营数据库或指标口径，详细证据见`docs/RELEASE_REVIEW_v1.0.149.md`。
