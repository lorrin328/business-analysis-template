...
- 连续观察读取`GET /api/market-analysis/observability?limit=6`；核对成功周期、运行尝试、首次成稿率、运行成功率、Pro升级率、时长、来源核验率和引用覆盖率。历史报告缺少运行字段时允许显示“待积累”，不得倒填虚构成本或耗时。
...
- 可用月份：`GET /api/honor/periods` 返回状态成功且已有人员汇总的历史批次，并补充最新业绩数据月份。最新月份尚无批次时显示“待生成”，不得因此回退到上月。
- 月份查看不写库。待生成月份由有 `honor_recalculate` 权限的用户点击“生成截至最新数据日的结果”；页面预填真实数据截止日，后端拒绝晚于该日期的过程测算。
- 最新日期复核：统一测算截止日来自 OTO、证保星钻适用业绩的最新日期；OTO、证保最近出单日分别展示。某条线最近出单日较早时，先核对是否只是当日无出单，不直接判定数据缺失。
- 人力门禁：OTO、证保目标月份人力数据均存在才允许生成最新月份；缺失时页面列明业务线，不以 0 人替代。
- 每次导入当月新业绩或人力后，进入星钻页确认最新月份和截止日；若当前批次落后，由有权限人员重新生成过程结果，再复核个人差额。
...
- 截止日验收：`meta.performanceCutoff`必须等于生产业绩表全业务最新覆盖日；`meta.lastBranchBusinessDate`为所选期间证保最后出单日，两者允许不同。无证保出单的已到达月份应返回0，不应返回“未来期间”。
...
- 运行数据：`/var/lib/business-analysis-market`；成功报告归档位于`reports/`，每次成功或失败尝试的安全运行摘要位于`runs/`；受保护配置为`/etc/business-analysis-market/market-analysis.env`。
...
- 模型路由：来源侦察`MARKET_ANALYSIS_SOURCE_SCOUT_MODEL=deepseek-v4-flash`；主研`MARKET_ANALYSIS_PRIMARY_MODEL=deepseek-v4-pro[1m]`；第一次修复`MARKET_ANALYSIS_REPAIR_MODEL=deepseek-v4-flash`；第二次升级`MARKET_ANALYSIS_ESCALATION_MODEL=deepseek-v4-pro[1m]`。候选来源默认4线程程序核验；`ANTHROPIC_DEFAULT_HAIKU_MODEL`和`CLAUDE_CODE_SUBAGENT_MODEL`均使用Flash。不得仅凭网传或单次时延全切单一模型；至少结合官方同模式评测和项目内首次成功率、修复率、总耗时、成本、最终质量进行判断。修改后先做Flash最小JSON调用，再用dry-run核对`modelPlan`，不得输出受保护环境文件全文。
