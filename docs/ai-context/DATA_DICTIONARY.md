...
## 2026-09-24 经代扩展字段（v1.0.159已上线）

`jingdai` 日常源新增 `当前缴别小类`、`保障期限`、`业务类型`、`产品大类`，沿用原始表自动扩列保存，已有明细API动态列目录可读取；不据此改变金额、长短险或产品手工分类。完整月替换保护已有非空维度，旧版缺列文件不得清空同月份的新字段。后台默认单文件上限100 MiB，显式环境变量优先。已发布并通过本批四文件导入与公网13列核验，最新导入记录346。
