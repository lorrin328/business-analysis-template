...
## 2026-09-05 增量导入按源覆盖范围清理，跨源指标从当前库联合重算

- 完整月替换的清理范围取输入源实际覆盖月份，不能取“有聚合结果”的月份；即使条件汇总为空，也必须清理该源对应的旧分区。共享汇总表只清理本次更新源，保留另一来源。
- 活动人力依赖人力和业绩两类源，任一类单独更新都应使用当前事务中的另一类源重算；业绩日期被纠正时同时覆盖旧、新统计月份。
- 普通同字段导入保留事务内一次完整历史活动证据扫描及回滚，移除重复预检；新增高优先级日期字段时保留额外历史预检。完全局部化连续挂零重算须另行证明跨月和历史缺日期边界，不在本次强行实施。
- 单页异步查询只允许最新请求提交数据或错误；上传文件及预检结果绑定同一任务，文件变化立即失效，结果不明时不复用确认。
- 账号同角色局部更新不重置自定义权限；目标配置必须先验证有限、非负，再删除和替换旧值。请求内认证复用不能变成跨请求缓存或绕过模块授权。
- 新增迁移 `20260905_incremental_partition_and_hr_consistency` 强制重建存量汇总，先在生产一致性副本核对，再保护生产原始数据和历史配置发布。

## 2026-09-04 市场研判最终证据对齐必须回到修复闭环

- 模块事实收敛为独立核验的原文摘录后，必须重新执行完整报告校验；此时出现的未支持数字、日期或政策状态不能绕过修复流程直接终止。
- 对齐后的证据错误允许额外一次定向修复，修复结果必须重新完成结构校验、来源独立核验、事实对齐和9分发布门槛，不通过时继续保留上一期报告。
- 不通过证据或质量门禁属于确定性内容失败，使用退出码2并由systemd禁止自动重启；网络、模型接口和进程异常继续使用退出码1，保留有限自动重试。
- 不通过简单放宽数字或日期规则解决问题；管理建议日期仍由模型在定向修复中改写或补足语义，外部事实数字和政策状态继续要求逐字证据锚点。

## 2026-08-26 职拓作为横向业务标识建立专用分析事实

- 业绩基表新增“是否职拓”后，职拓不替代原业务模式，也不从OTO、证保、蚁桥汇总中扣除；同一保单保留原业务模式并可同时进入职拓专题分析。
- 新增日级`agg_zhituo_performance`，保留机构、人员、产品、产品类型和交期维度，避免独立页直接扫描原始大表；迁移`20260826_zhituo_analysis`标记`requires_aggregate_rebuild=1`。
- 未取得正式职拓目标前，不虚构达成率目标。主看板卡片展示所选范围职拓期交保费、件数和同口径同比；独立页中的“达成”表示实际经营结果，不冒充目标达成率。
- 独立页复选年度、月份和机构，所有图表和明细共用同一筛选条件；承保件数使用源字段净额，不以数据行数替代。

## 2026-08-25 市场研判全部模型角色切换至Flash Vision实验模型

- 根据用户明确指令，Claude默认模型、Opus/Sonnet/Haiku映射、子任务、来源侦察、主研、首次修复和升级修复统一切换为`deepseek-v4-flash-vision-exp`，替代2026-08-14确定的Flash/Pro互补路由。
- 保留角色和两级修复流程，便于继续区分首次修复与升级修复；两级修复当前使用同一模型，不再将第二级显示为“Pro升级”。
- 结构化JSON Schema、独立来源复核、9分质量门槛、失败不覆盖上一期和凌晨1点每3个自然日到期检查均不改变。
- 该模型为DeepSeek官方实验模型；以官方Anthropic兼容、JSON输出和工具调用支持为启用前提，并通过运行台账持续观察质量、耗时和失败率。
- 本次模型与代码切换不涉及经营数据库结构或数据写入，按用户指令不执行数据库备份、Excel重建或聚合重建。

## 2026-08-25 星钻最新数据月份与历史测算批次分开

- `GET /api/honor/periods` 同时返回历史成功批次和最新可用数据月份。最新月份尚无测算批次时，以待生成状态进入月份选择，不因缺少批次而隐藏。
- 查看月份继续保持只读，不自动写库。有 `honor_recalculate` 权限的用户可明确发起“按最新数据生成过程结果”，避免打开页面即触发耗时计算或改写结果。
- 星钻统一测算截止日取 OTO、证保适用业绩的最新可用日期；页面同时列出各业务线最近出单日。某条线当天无出单不等于该条线数据缺失，不能将最近出单日误写成全局数据截止日。
- 当前月份的过程截至日不得晚于最新可用业绩日期。人力数据未覆盖目标月份时，不开放生成按钮并明确提示缺失业务线。
- 原始业绩按统计年度附近的索引范围读取，保留跨月承保、入账日期判定所需的边界记录，避免每次星钻测算将五百多万行历史记录全部载入内存。

## 2026-08-22 AI只读接口复用账号认证但不降低只读和权限边界

- AI只读接口推荐使用现有看板账号的HTTP Basic认证，解决人工维护独立Token易错、易过期和难以区分操作人的问题；也兼容网页登录后生成的会话Token。
- 账号认证不等于开放普通业务API。`/api/ai/*`仍只提供既有GET只读端点，KPI、机构、队伍分别校验账号的`kpi`、`org`、`team_enhanced`权限，综合快照同时要求KPI和机构权限。
- Basic认证不创建持久会话，避免每次读取产生无用Session；认证失败复用登录接口的IP+用户名限流和锁定，并记录脱敏失败审计，成功读取记录真实账号。
- 为避免中断定时市场研判等服务间调用，`AI_READONLY_TOKEN`保留为兼容通道，但不再是人工AI读取的必需配置；后续仍须按安全遗留要求轮换可能暴露的旧Token。
- 账号密码只允许通过HTTPS标准认证头传输，不得进入URL、提示词、代码、Git、日志或OpenAPI文档。
