...
## 2026-07-28 星钻联盟按四类业务任务组织页面

### 决策

星钻联盟一级入口固定为“总览、机构与项目、人员追踪、数据与规则”。机构和项目属于同一比较任务；会员、专员、管理职和月度预警属于同一人员查询任务；数据检查、重新测算和导出属于管理任务，不与日常查看并列。

### 边界

- 不改变个人、主管、经理分轨统计和数据截至口径。
- 不删除原有机构、项目、人员、预警和历史数据，只通过合并入口、折叠明细和默认字段降低首屏复杂度。
- 技术批次、规则版本和数据源说明保留在“数据与规则”，经营总览只展示业务可理解的结果。
