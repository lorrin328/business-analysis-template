...
- 【数据源待补】新版人力基表不再包含 `入职年`、`入职月`，当前星钻重算已安全降级且不再阻断，但“新星人力”暂不可计算。若需恢复，应由正式人力数据源重新提供入职年月，不从司龄区间反推。
- 【评估】业绩基表新增 `证券方营业网点名称`、`证券方销售人员工号`；当前仅保留原始字段。后续如需开展证保网点产能、集中度或证券方人员协同分析，应先确认权限、去重主键和统计口径后再接入聚合。

## 2026-07-14 审计整改

- 【P0/安全遗留】吊销聊天中已暴露的 DeepSeek API Key，生成新 Key 后以 root 可读、`market-analysis` 组只读的方式写入 `/etc/business-analysis-market/market-analysis.env`；用户当前决定继续使用原 Key，风险尚未消除。
- 【P0/安全遗留】轮换生产 `AI_READONLY_TOKEN`，同步更新主应用 `.ai_env` 与市场研判受保护环境文件，然后验证旧 Token 已失效。
- 【已完成 2026-07-22】服务器已安装 Claude Code `2.1.217`，验证 DeepSeek V4 Pro、WebSearch/WebFetch、项目Skill、证据门禁、失败检查点和 `OnUnitActiveSec=3d` timer；首期报告 `market-20260722-153346` 已发布。
- 【P1/观察期】首期已完成；继续检查后两期的来源覆盖、微信公众号可达率、重复模块、历史反转判断和条线行动可执行性，再决定是否将 Skill 转为稳定观察期。
...
- 【部分完成 2026-07-20】缺表、空明细和公式缓存缺失的方案底稿已拒绝写入成功批次；正式规则引擎仍不得长期依赖 Excel 缓存值。
- 【展示侧已完成 2026-07-20】生产无正式目标时，目标类 KPI 显示“目标待配置”且不形成领先/落后判断；后续仍需由服务端统一输出指标可计算状态。
- 【部分完成 2026-07-20】已启用登录失败限流/锁定，密码重置撤销旧 Session；生产公开注册按用户既有决定继续开启，仍需评估审批制、HTTPS、密码强度和 Session 定期清理。
- 【P2】扩展数据质量检查到月度、机构、产品、交期、价值、目标、导入批次、机构范围外数据和方案批次；健康检查拆分服务健康与业务就绪。
- 【部分完成 2026-07-20】已建立 `rangeType + startDate + endDate` 统一期间模型并清理交期月级限制；仍需决定跨来源混合指标最终采用“各来源真实截止”还是“共同最早截止”。
- 【P2】对固定十家机构范围外数据增加异常提示和勾稽说明；明确平台总量与机构表的范围差。
- 【已完成 2026-07-20】生产 seed 文件已清空经营数值，仅保留运行空容器；如需开发 mock，应另建不进入生产包的数据源。

## 2026-07-09 组织发展方案计算

- 【高】补齐“2026年组发政策”正式结算所需字段：推荐人/推荐关系、推荐人当月活动人力、回执回访、犹豫期、45 日内撤保退保、自保互保、活动主管/经理状态、原主管/原经理归属。
- 【高】修正业务底稿参数：`参数表!B2` 应按正式通知口径调整为方案期起始日 `2026-07-01`，避免把历史入司人员计入方案统计人力。
- 【中】核对并修正 `入职主管!AR` 经理参与方案公式，当前疑似在 `入职经理!A:F` 中取第 7 列导致结果失效。
- 【中】确认组织育成奖页面展示口径：当前底稿挂在晋升主管行，正式通知口径为发放给晋升人员原主管或无主管时原经理。
- 【中】如需从底稿展示升级为自动结算，应基于保单、人力、推荐关系等源字段重写原子计算，不应长期依赖 Excel 缓存公式值。
- 【低】新增第二个方案时，优先扩展 `backend/scheme/config.py` 方案列表和独立解析器，复用现有 `/scheme-calculator.html` 方案选择页。
...
- 【部分完成 2026-07-20】systemd 生产库已迁至 `/var/lib/business-analysis/business_data.db` 并接入一致性备份；仍需补充自动化恢复演练，并覆盖 Docker `/data/business_data.db`。
