...
- 【P0/立即】移除 `www-data` 对 root 部署脚本的写权限；使用 root-owned 固定 wrapper，应用代码只读，仅数据库和日志目录允许写入。
- 【P1】把 SQLite WAL 备份改为 Online Backup API/`.backup` 或停服 checkpoint 后备份，并完成一次自动恢复演练。
- 【P1】为 legacy 只读 API 补齐与新接口一致的模块权限，增加权限绕过回归测试并制定下线期。
- 【部分完成 2026-07-20】v1.0.103 页面已改名并明确为“方案底稿复核、非最终发放结果”；仍需补齐缺表/空明细/公式缓存缺失拒绝逻辑，正式规则引擎不得依赖 Excel 缓存值。
- 【展示侧已完成 2026-07-20】生产无正式目标时，目标类 KPI 显示“目标待配置”且不形成领先/落后判断；后续仍需由服务端统一输出指标可计算状态。
- 【P1】关闭生产公开注册或改审批制；启用 HTTPS、登录限流/锁定、密码强度、Session 清理；密码重置时撤销旧 Session。
...
- 【P2】生产构建移除真实/近真实 seed data，开发 mock 独立打包且不部署。
