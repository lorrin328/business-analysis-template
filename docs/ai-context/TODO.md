...
- 本轮 v1.0.146 完成完整月/业绩补充保护、上传预览确认、客户事实增量刷新、指标共同合同、审计失败退出、镜像白名单、依赖安全更新及冻结恢复链；发布验收结果见 `docs/RELEASE_REVIEW_v1.0.146.md`。
- Windows 发布工作区改用同步目录外的独立 Git 克隆；原同步目录的既有修改与冲突副本保留。不要将 `.git`、虚拟环境、SQLite及WAL交给跨机文件同步；历史冲突不能以文件新旧时间直接覆盖。
- 备份容量/保留策略已进入部署脚本；独立介质副本及恢复演练按本轮发布记录验收，下方“仍需治理”是整改前记录。
