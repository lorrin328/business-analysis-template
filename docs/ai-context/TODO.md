...
- 【高】根分区98GB中备份目录一度占用62GB；本次4.99GB在线备份将根分区写满并使SQLite无法创建WAL。应为`deploy.sh`增加备份前可用空间门槛、备份保留数量或容量上限，以及失败临时文件清理，且必须先保留至少一份已验证可恢复的完整备份。
- 【现状】已删除本次发布新生成的`business_data.db.20260819_152348`及其元数据，以及2026-08-14遗留的一份未完成临时备份；最近完整备份仍为`business_data.db.20260817_085916`，根分区当前可用7.9GB、使用率92%。
- 【验收要求】治理后需覆盖“空间不足时在复制代码和停服务前失败”“不删除唯一有效备份”“临时文件失败清理”“备份元数据完整性”和“服务可继续创建WAL”测试，并在Ubuntu真实目录做只读容量预演。
