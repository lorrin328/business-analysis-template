...
- 核对2024年蚁桥active=60与avg_headcount=47.54的期间/人员边界，说明活动率1.2621的来源；不直接按异常提示改数据。
- 定位2025年机构日级规模保费汇总40,135.85万元与重算40,135.84万元之间0.01万元差异，分别复核聚合精度、求和顺序及源数。两项均已在本次导入前冻结库复现，详见 `../RELEASE_REVIEW_v1.0.159.md`。

## 2026-09-07 三维审查后续

- 【已完成】v1.0.152已通过PR #29发布Ubuntu主应用，方案复核线上页面、导航、接口和权限已移除。
- 优先修产品件数金额兜底、人力机构空选、客户跨标签旧缓存；随后处理单位、Top20截断、经代10年日来源判定、失败态旧KPI和网点固定数字。完整证据/责任/节奏见`docs/REVIEW_20260907.md`。
- 观察下次市场研究是否生成有证据的产品模块或明确缺口；本次只完成CC真实最小调用及dry-run，未运行整轮研究，不能以配置验收替代产品报告验收。

## 2026-09-05 v1.0.150 复审修复状态（优先于下方历史条目）

- 【已上线并验收】本轮 7 类缺陷及重复扫描、上传进度刷新、请求内认证优化已完成，Windows 810 项、Linux 811 项回归、生产逐表核对及公网验收通过，发布状态 accepted。强制迁移从当前库原始数据修复历史汇总，补回 2024 年活动人力 3,193 人月；依据与边界见 `docs/RELEASE_REVIEW_v1.0.150.md`。
- 【后续独立优化】登录失败记录的有界过期清理、年度产品汇总增量语义、连续挂零完全按受影响范围重算；先明确跨月、历史缺字段和事务失败边界，再验证收益，不借优化扩大本轮重构。
- 【持续保护】保持真实零、缺失、未关联和不可计算状态；历史 8 组保单键歧义继续等待权威映射，不按相近编号自动补配。独立备份更新仍需纳入后续运维。

## 2026-09-02 稳定性版本状态（优先于下方历史条目）

- 【已上线并验收】本轮 v1.0.146 完成完整月/业绩补充保护、上传预览确认、客户事实增量刷新、指标共同合同、审计失败退出、镜像白名单、依赖安全更新及冻结恢复链；654项Linux回归通过，GitHub、GHCR及Ubuntu均完成，完整复核未触发回退。证据见 `docs/RELEASE_REVIEW_v1.0.146.md`。
- Windows 发布工作区改用同步目录外的独立 Git 克隆；原同步目录的既有修改与冲突副本保留。不要将 `.git`、虚拟环境、SQLite及WAL交给跨机文件同步；历史冲突不能以文件新旧时间直接覆盖。
- 【已完成】备份容量/保留策略已进入部署脚本；独立Windows介质副本已恢复并通过哈希与全库完整性检验，本次冻结备份及恢复包保留。下方“仍需治理”是整改前记录；后续需将独立副本更新纳入日常运维。
- 【已修复、保留历史】原仓库损坏Git pack已精确恢复并通过完整性检查，损坏原件及用户stash保留；历史同步冲突仍待逐项核对，不能批量删除。
- 【数据待补齐】8组历史保单候选键歧义仅作原号精确关联，待权威映射补齐；当前未关联金额仍保留，相关客户结论明确覆盖范围。
- 历史 GitHub、DeepSeek、AI只读凭据及 SSH 口令的吊销/轮换没有新的平台证明，不标记完成；不把凭据值复制到项目。不得为了本轮发布撤销用途不明的现用凭据。
- 后续经营督导工作台单独迭代：先依本次统一指标合同形成缺口、动能和数据覆盖视图，再接责任、措施、期限、复核；正式责任关系、目标及继续率源缺失时仍不推定。


## 2026-08-30 项目接手核对

- 【安全优先】撤销本轮聊天中暴露的GitHub Token并轮换；只通过受保护凭据存储配置新值，验证旧值失效，不向聊天、代码或项目记忆提供新值。
- 【开发前置】为当前macOS准备独立的Python 3.10+环境并安装项目测试依赖；现有Windows `.venv`不跨平台复用。完成后运行正式测试集，不能引用历史通过数代替当前验证。
- 【提交前置】核对381个既有工作区差异中的执行位和行尾变化，并比较19份同步冲突副本；先确认真实内容与权威副本，再单独治理，不批量覆盖或清空。
- 【文档待核验】本地已为v1.0.142，现有生产日志最近明确记录v1.0.141；后续获授权验收生产时核对实际版本。逐项标注TODO/OPEN_QUESTIONS中已被替代的交期日级、模型路由及Webhook历史状态，不依据旧条目恢复功能。

## 2026-08-19 生产备份容量治理

- 【容量风险已解除并于2026-08-22复核】应用、数据库和历史备份目录均已bind mount到2TB`/data`盘；本次发布前约1.9TB可用，4.99GB在线备份、完整性校验和服务WAL均正常，根分区不再承载新增生产备份。
- 【仍需治理】为`deploy.sh`增加备份前可用空间门槛、备份保留数量或容量上限、失败临时文件清理，并保证不删除唯一有效备份；大容量盘不能替代保留策略。
- 【仍需独立灾备】当前运行库和在线备份位于同一`/dev/sdb`故障域，需建立PVE、极空间或其他独立介质副本并完成恢复演练。

## 2026-08-14 市场研判9分观察期

- 【本期基线】首期9分门禁生产报告 `market-20260814-121537` 得分9.6；证据、覆盖、滚动和行动四维满分，因先Flash后Pro升级修复，运行可靠性为0.6/1.0，完整运行约35分钟。
- 【第二份样本】`market-20260814-171105`于16:35:05启动、17:11:12发布，完整运行36分07秒；Flash侦察、Pro主研后由Flash首修通过，未触发Pro升级。最终分数须从已登录页面或受保护观测API查看，但发布已保证不低于9.0。
- 【模型结论】官方能力数据与两份生产样本均不支持全部切换Flash；继续观察至少六个成功周期。若首次成功率低于80%、Pro出现兼容失败，或Pro新增质量不足以覆盖耗时与成本，再使用同证据包对照测试Flash主研方案。
- 【连续六期】记录每期质量分、首次成稿是否通过、Flash修复、Pro升级、外部来源验证率和完整运行时长；首次成功率目标不低于80%。
- 【已完成】成功报告固化运行遥测，成功/失败尝试写入`runs/`，页面和API自动汇总最近六期质量、首次成功、运行成功、Pro升级、时长、成本和来源贡献；旧期不倒填未知数据。
- 【已完成】已上线Flash前置来源侦察和程序并发预验证，且未降低TLS、域名、标题、主体或摘录匹配门槛。
- 【连续观察】连续六期比较候选通过率、完整时长、首次成稿率、Flash修复率和Pro升级率，确认是否达到降本提质目标。
- 【公众号观察】记录每期候选数、主体核验通过数和失败类别；有效数可以为0，不以转载、搜索摘要或访问控制绕过补数。
- 【来源观察】每个同业模块一手证据保持100%；微信公众号公开可核验来源仍为0时如实披露，不以转载补数。
- 【行动闭环】从第二期新合同报告开始，检查稳定`actionKey`是否正确续接，持续行动必须更新进度、验收结果或调整原因。
- 【已完成】到期行动不得静默顺延；研究提示词要求先记录验收结果、未达节点或证据缺口，页面自动提示到期及七天内待复核行动。
- 【人工抽检】每期随机抽取至少2个模块，人工打开原文复核事实、判断、影响和失效条件；程序9分不能替代业务及合规签审。

## 2026-08-02 客户口径核对与后续扩展

- 【待用户核对】按`docs/客户分析指标口径核对报告_v1.0.127.md`确认系统新客、业务新客、同日多单、持单范围、退保率和成熟队列边界。
- 【建议优先】确认后增加业务迁移、首购—复购产品矩阵、复购层级和成熟12个月队列比较。
- 【数据依赖】正式13/25月继续率仍需应缴、实缴、失效/复效和观察点数据；当前状态快照不得改名为继续率。
- 【运维观察】首批真实大文件客户清单网页导入后复核分片续传、临时磁盘峰值、后台处理时间、旧快照跳过、匹配率变化、写锁时长、服务内存和24小时过期清理。

## 2026-08-02 全量库性能观察

- 【本次完成】消除队伍增强全量明细读取、产品日期表达式主扫描和首屏重复请求；增加人员月度、产品日级聚合及大库整库演练。
- 【上线后观察】连续观察主看板接口耗时、应用进程内存、SQLite锁等待和部署聚合重建时长；只有聚合后仍不能满足并发或维护窗口要求时，重新评估MariaDB。

## 2026-08-01 客户分析后续数据

- 【高】如需正式13个月或25个月继续率，接入保单应缴、实缴、续期、失效恢复及观察点数据，单独建设继续率事实；不得用当前状态快照倒推。
- 【中】下一次客户清单导入时保留批次变化，用于识别有效、停效、退保等状态迁移；当前单批快照只能回答数据截止日状态。
- 【中】持续观察全量SQLite候选构建时长、运行库体积、备份时长和线上查询延迟；只有达到既有架构评估触发条件后再决定是否迁移MariaDB。

## 2026-07-29 证保网点分析

- 【本次完成】接入受保护网点主数据、动态经营分析、权限、页面、主导航和生产导入链路。
- 【待业务确认】补充网点状态、生效日期、别名映射和正式责任人关系后，再扩展动态在营分母和责任覆盖分析。
- 【待数据源】价值、品质、继续率和费用当前没有可追溯的网点键，不做网点级分摊。
- 【待映射】取得广发主网点到86个转介绍子网点的权威业绩映射后，再计算子网点活动率、排名和网均。
