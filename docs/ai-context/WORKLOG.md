...
## 2026-08-17 星钻底层区分外勤管理职与专员

- 新增底层 `staff_category` 字段，将人员岗位类别 `专员` / `外勤管理职` 与荣誉身份轨道 `个人` / `主管` / `经理` 分开；管理职人员的个人轨道不再被当作专员。
- 字段已贯通人力源快照、月度计算结果、年度人员汇总和 dashboard 派生数据；机构会员结构、专员历史、会员清单及预警均按岗位类别识别。
- 增加幂等数据库迁移，对历史批次按同批次人力源记录回填岗位类别，不改动历史钻石余额和会员等级。
- 本地库最新成功批次 `batchId=11` 回读为会员身份轨道 `59` 条，其中专员 `51`、外勤管理职 `8`；原身份轨道仍为个人 `53`、主管/经理 `6`，据此确认有 `2` 条管理职个人会员轨道此前会被误归入专员。底层三张表岗位类别均已回填，数据库 `quick_check=ok`。
- 荣誉专项测试 `37 passed`、全量回归 `371 passed`；JavaScript语法、Python语法和 `git diff --check` 通过。本次未提交、未推送、未部署生产。
