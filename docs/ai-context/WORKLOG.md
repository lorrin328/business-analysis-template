...
- 验证：新增部署计划与发布顺序测试，覆盖既有库快速路径、新强制迁移、人工强制聚合、首次Excel建库、源文件不足、首次空库、在线备份前置、健康检查和Nginx reload；专项13项通过，Ubuntu端`bash -n`、Python编译及快速/强制迁移计划预演通过。全量395项中392项通过，3项既有产品结构测试因隔离工作树本地库缺少转型业绩基线数据失败，与部署改动无关。生产发布耗时、停机窗口和数据完整性待可信归档部署后补录。
