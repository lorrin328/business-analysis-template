...
- GitHub：功能提交 `6bbd79a feat: support multi-month dashboard filters` 已推送分支 `codex/team-high-productivity-bands`，并更新草稿 PR `#1`。
- 生产部署：使用提交 `6bbd79a` 的可信归档部署至 `192.168.50.6`，设置 `REBUILD_DATABASE=0` 保留生产库；部署前 SQLite Online Backup 为 `/opt/business-analysis-backups/business_data.db.20260727_113623`，备份元数据记录 `integrity_check=ok`、`quick_check=ok`、`table_count=38`。
- 生产验收：内外网健康检查均返回 `v1.0.110`、最新期间 `202607`、`table_count=38`、缺表为空；`business-analysis` 与 nginx 均为 `active`，nginx 配置正常，部署后 error 级 journal 为空，生产数据库 `integrity_check` 与 `quick_check` 均为 `ok`。
- 生产复算：2026 年 4 月+6 月非连续组合下，产品结构组合总额 `21924.44` 万元、逐月相加 `21924.47` 万元；交期结构组合总额 `21924.46` 万元、逐月相加 `21924.44` 万元；差额均为展示层四舍五入。OTO+证保队伍返回所选月份 `[4,6]`、去重样本 `437` 人、累计期交保费 `5283.06` 万元、标准人力期间月数 `2`。
- 页面验收：生产页面实测四个模块在月度下均可复选非连续的 4 月、6 月，在季度下均可通过 Q2 选择 4—6 月；机构表头、产品排名、交期明细和队伍结构与产能分析同步更新，高产能六档联动正常。390×844 视口下页面无横向溢出，登录后的干净重载无 error/warning；验收创建的临时普通用户及其 Session 已删除。
