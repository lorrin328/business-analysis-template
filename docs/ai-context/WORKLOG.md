...
## 2026-07-28 v1.0.109 星钻联盟页面精简

- 任务：在不改变星钻计算、分轨统计和数据截至口径的前提下，降低页面入口、指标和文案复杂度。
- 页面：九个一级页签收拢为“总览、机构与项目、人员追踪、数据与规则”四个入口；机构/项目、人员/预警和管理操作分别归并。
- 展示：首屏只保留五项核心结果；会员结构下沉至总览；新入会和晋级默认只展示前八条，完整人员查询统一进入人员追踪。
- 文案：清理“驾驶舱、追踪池、聚焦、识别、转化”等泛化表达，并将技术表名、批次号和规则版本收进数据页。
- 交互：人员追踪统一筛选并显示结果数量，待跟进人员默认展开，会员、专员历史和管理职历史按需展开。
