...
- 用户明确授权同步GitHub、部署Ubuntu，再导入原每日导入文件夹的全部Excel。基于最新v1.0.158发布3f3a423；Windows938项通过/3项跳过、Node57项通过，GitHub两条流水线成功。
- Ubuntu保库发布accepted，49张业务表与冻结库逐行一致，跨盘恢复副本哈希一致。正式公网版本、24项静态资源、鉴权及业务API通过。
- 四份Excel正式HTTP导入成功，0错误/0跳过，最新记录346；446,442行源字段逐值核对一致，经代13列已落库。
- 2026审计0问题；2024活动率提示、2025机构日级规保0.01万元差异在导入前冻结库同样存在，未擅自修正或降低审计要求。详情见 `../RELEASE_REVIEW_v1.0.159.md`。

## 2026-09-24 经代新版 Excel 本地适配

- 核验原每日导入目录经代文件：382,997 行、13 列、2022-01 至 2026-09 共57个月，新增缴别小类、保障期限、业务类型、产品大类；现有解析器可保留全部字段。
- 修复文件超过后端20 MiB默认限制的问题，默认提高至100 MiB并保留环境覆盖；补齐完整月替换的经代维度字段丢失保护。
- 56项相关测试通过，真实文件现有聚合链成功；未改变金额或分类公式，未导入运行库、推送或部署。详情见 `../JINGDAI_EXCEL_REVIEW_20260924.md`。

## 2026-09-22 公众号信息保留与交叉比对上线

- 用户明确要求“可以做交叉比对，但是不能完全否定公众号采集的信息”。据此调整项目局部策略，不修改全局Skill：官方原文继续核验，未能直接核验的摘要、片段、作者观点和转载线索保留，主研进行交叉比较。
- 新增wechatLeads与wechatLeadReviews，前置核验失败不自动判错；比对标签需引用实际核验通过的其他外部来源，完全交叉支持还需A/B依据。重复转载不能当作独立支持。页面分别计数和展示正式证据与公众号参考。
- 可信提交8caebf0于17:24定向部署5个文件；恢复点 `/opt/business-analysis-backups/wechat-evidence-20260922-172401`。108项后端与8项页面交互测试通过；market-ai真实Hermes一次检索保留1项摘要、0项正式证据，未读全文及主体缺口明示。实测不代表摘要业务内容已获证实，也未生成整期新报告。
- 公网HTML/JS字节匹配发布归档；本机/公网健康、匿名401、已鉴权经营快照200通过；主服务和触发器active，原latest/status哈希不变。未改NAS或Hermes权限、未重启主看板或重建数据库。规则从新一期研究起生效，既有报告不回写。

## 2026-09-22 Hermes公开资料采集接入

- 用户明确将助手公开资料采集从OpenClaw转向Hermes，并确认后台定时市场研判也接入；原自动任务并无OpenClaw调用，本轮是新增辅助来源通道。
- 新增Hermes客户端、助手入口、Windows用户绑定加密凭据读取；后台只发送固定公开研究任务，候选经原独立核验，不发送经营快照和历史台账。保留原来源渠道、模型路由和发布门槛。
- 真实健康、鉴权、能力查询及公开网页采集调用成功；102项相关测试通过。Hermes服务工具权限较宽，只读限制是调用指令，不是服务器隔离。
- 用户补充提权凭据后，2026-09-22 16:50从可信提交02dad37定向部署3个组件并配置Hermes；恢复点 `/opt/business-analysis-backups/hermes-research-20260922-165035`。未重启主看板、未重建数据库、未修改NAS或全局Skill。
- 主服务、定时器、手动监听均active；本机/公网健康与市场静态资源200、匿名接口401、已鉴权经营快照200且非空；代码与发布归档字节一致，原latest/status哈希不变。
- market-ai实际Hermes小样例结构化返回成功，3次查询、0候选；完整采集样例未通过验证，原因未充分定位，未宣称公众号正文成功读取或新整期报告生成。完整定时效果及来源贡献待观察。
- 原项目Git HEAD/index受只读保护，未修改权限；在独立发布副本提交、102项测试通过并生成归档，提交已取回本项目codex/hermes-market-release分支，工作区仍master且保留既有改动。未推送GitHub。详见 `../HERMES_RESEARCH.md`。

## 2026-09-12 v1.0.155 GitHub推送与Ubuntu发布完成

- 用户明确授权“推送、部署”。代码 `38d8ecd` 已推送master，Windows888项通过/3项跳过，GitHub Linux891项通过、依赖无已知漏洞，应用验证及镜像发布成功。
- 从校验一致的可信Git归档保库部署，发布编号 `20260912_230620-302600`，主服务23:10:23停止、23:12:54恢复。无依赖、迁移及聚合变更，不导入Excel。
- 公网四类明细与字段目录、授权/拒绝、分页首尾样本、请求头筛选及OpenAPI通过。线上业绩38列、5,159,244行，其余三表9/17/8列；49张业务表与冻结恢复库全行有序哈希一致，三年KPI及市场报告文件不变。
- 441个归档文件、23项公网资源一致，root:root且应用账号不可写；服务正常、无异常重启或warning日志。浏览器显示v1.0.155登录界面，未做登录后浏览器交互；临时API验收账号及关联记录已清理。
- 约5GB冻结库与系统盘独立副本哈希相同，integrity_check/quick_check通过；独立副本位于 `/var/backups/business-analysis-independent/v155-20260912`，为同机跨盘备份。发布已accepted，详见 `docs/RELEASE_REVIEW_v1.0.155.md`。

## 2026-09-12 四类日常Excel明细API（本地候选，未发布）

- 用户要求先核验Excel是否保存全字段，再开放API，并确认仅覆盖四类日常导入。实际四份Excel有效业务字段34/9/17/8，在本地原始表全部存在；最新文件行数与本地库不同，未导入或据此宣称最新数据已完整落库。
- 新增 `/api/ai/raw-datasets` 与 `/api/ai/raw-data/{dataset}`，动态读取全部原始列和期间，支持选列、精确筛选、最多1000行的游标分页及已落库日常导入ID变更检查。保留SQLite值与空值，说明原始单位、当前存量及非跨请求快照边界。
- 新增 `ai_raw_data` 权限及管理界面勾选项，管理员允许，其他角色默认拒绝；现有汇总Token不扩权。Basic/会话复用现有认证与限流。筛选通过ASCII转义JSON头传递，审计不含筛选值或明细。
- 专项及相关回归201项通过；真实本地库副本203页、200,139行全部字段内容与独立SQL全量哈希一致，quick_check通过；JS语法、前端脚本哈希、diff检查通过。
- 未发布GitHub、未部署Ubuntu、未改动聚合口径或导入数据；主应用发布版本仍为v1.0.154。证据见 `docs/EXCEL_API_AUDIT_20260912.md`，使用说明见 `docs/AI只读接口说明.md`。

## 2026-09-11 v1.0.154 UI 修复、GitHub 与 Ubuntu 发布

- 用户授权修复并同步 GitHub、部署 Ubuntu。CSS 打包白名单及 KPI 九项响应式排列修复完成；Linux 测试额外修复临时目录 root 所有权依赖，生产权限规则不变。
- 主助手实施、发布和终审；独立子任务复核 CSS 排除边界、Linux Docker 测试环境与布局断点。GitHub 最终代码 `86b5bd2`，Linux 863 项通过，镜像验证及发布成功。
- 按可信 Git 归档保库部署，版本 v1.0.154；未重建数据或聚合。主服务 23:37:05 停止、23:41:03 启动；本机、公网、鉴权、资源和业务数据核对通过，市场报告未变化。
- 51 张业务表、18 张聚合表及三年 KPI 业务响应一致；KPI 排除请求级 updatedAt 后，使用旧代码与冻结库只读复算比较，不以变化的请求时间判作业务变化。
- 冻结恢复点与系统盘跨盘副本完整哈希一致，SQLite 检查均 ok，已 accept-release。该副本是同机跨盘，不是异机备份。浏览器登录入口和本地静态布局通过，未重跑完整登录后交互。完整证据与恢复路径见 `docs/RELEASE_REVIEW_v1.0.154.md`。

## 2026-09-07 v1.0.152 GitHub合并与Ubuntu主应用发布

- 用户明确要求提交GitHub并部署Ubuntu。PR #29已合并为da4396d，Linux842项测试、依赖扫描、文件边界、镜像启动与Docker镜像发布全部通过。
- 使用合并提交可信归档执行标准deploy.sh，发布编号20260907_145940-275477；保留生产库和现有聚合，无新增迁移，不以Excel重建。服务15:02:37停止、15:06:37恢复，约4分钟用于最终冻结与完整性校验。
- 方案复核已在线上完整移除：导航/权限消失，页面/脚本/查询/上传在管理员下均404。市场产品专题与三项市场UI修复生效，旧报告兼容；原模型、调度与上一有效报告保留。
- 正式公网22脚本与页面、全部430归档文件一致；匿名401、授权200，实际Chromium加载KPI与产品页签、390px布局通过，临时会话清理，运行错误和异常重启均0。
- 冻结库和生产库51张业务表计数、18张聚合表全行内容、2024—2026 KPI全部一致；2026严格审计0问题。独立副本存放D:/business-analysis-backups/20260907/v152，完整SHA256、integrity_check、quick_check均通过，15:23:05已执行accept-release并确认为accepted；详细证据见docs/RELEASE_REVIEW_v1.0.152.md。

## 2026-09-07 项目三维审查、方案复核删除与寿险产品专题

- 按work入口和复杂任务分工，主助手规划、市场改造及最终验收；独立子任务分别完成UI审查、计算/字段审查、方案复核删除。审查发现11项问题，市场历史竞态、刷新丢选择、空样本0%三项本次修复，其余8项见`docs/REVIEW_20260907.md`。
- 本地删除方案复核导航、页面、脚本、API、专属计算包和权限；保留历史数据库表、旧权限记录与发布历史。旧URL和上传接口404回归通过。没有发布主应用，该删除尚未在线上生效。
- 市场新增寿险产品专题和10项证据字段，至少3类产品搜索主题，产品保证/非保证利益分列；缺口明确披露，旧报告兼容。独立复核发现否定语义截取和缺id异常后修复，并补回归；草稿、来源后和publish三道产品校验保护上一有效报告。
- 完整回归837 passed、1 skipped；最终变更专项65 passed，包含6项Node异步测试。桌面/手机合成界面渲染、JS语法与发布边界检查通过。实际调用work及寿险市场/产品技能，未修改全局Skill。
- Ubuntu仅定向部署已提交9e4721c市场组件并启用产品研究开关；保留现有DeepSeek实验模型。真实CC调用、只读快照/历史上下文、公网HTTPS静态文件、401鉴权和授权API通过；临时测试会话已移除。主服务不重启，timer/path恢复active，经营数据未重建。
- 组件恢复点`/opt/business-analysis-backups/market-products-20260907-1788761423`，组件已accepted；完整报告尚未运行，latest/status未变化，最新仍为market-20260904-150238。下一次研究生效，不能称已生成产品研判。

已确认混合历史月份和完整时间戳解析漏数，同时修复整数等值浮点年月解析。补充16项回归与强制重建迁移，Linux完整回归827项通过，依赖扫描及镜像发布成功。PR #28合并至fc8d8d9后以可信归档保库部署Ubuntu，正式公网活动率恢复19.6%（79/403.5）；独立SQL核对63个渠道月份一致，生产KPI与演练一致，严格审计零问题。副本33张受保护表逐行不变；8条派生歧义记录仅更新时间变化。公网登录、鉴权、22个静态脚本通过，独立Windows冻结备份哈希及数据库完整性检查通过。详见docs/RELEASE_REVIEW_v1.0.151.md。使用work技能完成规划、生产取证和验证。

本次发布于2026-09-07 11:29:54（北京时间）正式accepted，保留生产冻结恢复包及独立Windows副本，清理本次远端传输临时文件。

# 工作日志

## 2026-09-05 v1.0.150 项目复审、修复与保库发布

- 全面复审后修复 7 类问题：空条件汇总残留、单独更新人力/业绩导致活动人力失真、客户上传重入、迟到请求覆盖当前筛选、产品机构空选误查全部、同角色账号局部更新重置自定义权限、目标非有限数值破坏返回。同步减少普通导入的重复历史扫描、局部更新上传进度、按单次请求复用认证。
- 在同步目录外的独立 Git 克隆实施并回归，未覆盖原工作区既有内容。Windows `810 passed, 1 skipped, 3 warnings`，Linux CI `811 passed, 4 warnings`，依赖安全扫描、容器构建和受保护接口检查通过；实际 Chromium 合成接口验证上传分片与重入、文件变更失效、请求顺序和空机构筛选。
- 生产一致性副本完整重建耗时 887.25 秒，32 张受保护表逐行保持不变。2024 年总体活动人力 36 行、机构活动人力 188 行恢复，合计各补回 3,193 人月，独立 SQL 从原始明细复算一致；9 个经代日级金额出现现有精度的舍入末位变化，Decimal 独立核验误差不超过 0.5 元。严格审计零问题，未放宽审计阈值。
- 功能提交 `1d6fbc3` 经 PR #27 合并为 `4582251`，主分支验证及 GHCR 镜像发布成功。以该提交可信归档执行 `REBUILD_DATABASE=0 REBUILD_AGGREGATES=auto`，由新增迁移强制重建现有库汇总；16:21:03 停服、16:39:27 恢复，维护窗口 18 分 24 秒，未回退。
- 正式公网 v1.0.150、静态资源、匿名 401、敏感路径 404、登录后业务 API 和越权 403、实际页面筛选均通过；测试账号及附属记录清理后，账号、会话、权限、目标和历史批次仍与冻结库逐行一致，服务零重启、日志无错误。Windows 独立冻结备份通过哈希及实际 SQLite 完整性检查。
- 最终生产 32 张受保护表逐行不变，18 张汇总及客户事实与演练逐行一致，完整性与快速检查均为 ok；16:57:54 发布状态确认 accepted，保留冻结库、完整恢复包及独立 Windows 副本。完整验收及恢复点见 `docs/RELEASE_REVIEW_v1.0.150.md`。

## 2026-09-04 v1.0.149 市场研判最终证据闭环修复

- 生产只读诊断确认：凌晨1点自动研究在41分钟后因B2模块引入未支持的`5.8%`失败；11:35手工研究在21分钟后因M10模块引入未支持的`9月`失败。模型与来源调用已执行，终止原因不是API鉴权、定时器或服务器资源，而是报告证据门禁。
- 根因是程序先通过草稿校验，再独立核验来源并将模块事实替换为最接近的原文摘录；替换后判断或影响中的数字/日期可能失去同模块证据锚点，而最后一次`validate_report`位于修复循环之外，导致确定性失败被systemd重复运行。
- 修复为：最终事实对齐后捕获完整校验错误，同一次任务内额外允许一次定向修复；修复结果重新完成草稿校验、全部来源核验、事实对齐及9分门禁。内容门禁最终失败返回退出码2并配置`RestartPreventExitStatus=2`，网络或模型接口异常仍按退出码1重试。
- 页面将带`validationErrors`的失败明确显示为“报告未通过发布门禁”；定时启动前先清理旧failed状态。DeepSeek模型组合、证据等级、9分门槛、历史报告保护和经营数据库均不改变。
- 本地验证：市场研判专项`47 passed, 1 warning`，完整回归`732 passed, 1 skipped, 3 warnings`；Python编译、JavaScript语法和`git diff --check`通过。功能提交`d3c6cdb`经PR #26合并为`4b758a8`，GitHub验证`33845415599`、`33845375293`均成功。
- 可信归档SHA256为`e551f996bc833731b0df6b49e9bcaf7df53f99023eaa1cda521905c1db0545b9`。采用完整保库流程部署，未重建经营数据库或聚合；生产从原`verify`失败断点续跑，5分35秒后发布`market-20260904-150238`，质量评分9.4、11个模块、26个已核验来源，断点已清除，任务零重启成功退出。
- 正式公网健康、页面、匿名401和一次性管理员受保护接口验收通过；临时用户、会话和审计痕迹已清零。定时器与手动监听active/enabled，系统无failed单元；核心表行数保持不变。在线备份和冻结库完整性、快速检查均ok，最终发布状态accepted。详细证据见`docs/RELEASE_REVIEW_v1.0.149.md`。

## 2026-09-04 v1.0.148 证保网点太平人员展示与正式发布

- 用户要求在证保常规网点表中补充我司人员表现。页面已在“券商人员”后增加“太平人员”，展示当期有承保业绩的太平人员数；两类人员均按各自人员代码去重，后端沿用既有`internalAgents`字段，不改变数据库和其他经营指标。
- 功能提交`5cf2a65b84ddc23a9c04eff03ca8baf2c2149c70`；跨平台脚本摘要校验修复后，最终应用提交`5490a546cee29e2bbd2e9977b8281816be5a6b60`。Windows回归730项通过、1项跳过，36项Node检查通过；GitHub应用验证`33839312086`和镜像运行`33839312237`成功。
- 可信归档SHA256为`7f39c44694a84e0d1e8357a1b1685c8430f91ad000311308f84c4e07a47628b5`。以`REBUILD_DATABASE=0 REBUILD_AGGREGATES=auto`保库部署，核心表行数前后完全一致，全库完整性、快速检查和2026年严格数据审计均通过。
- 正式NPM公网与FastAPI本机版本均为v1.0.148，归档、服务器和公网脚本哈希一致；匿名业务接口401、敏感路径404。浏览器登录后确认147/147个网点、数据截止2026-09-04、“券商人员—太平人员—活动月”表头顺序及首行16—13—7实际数据，控制台无错误，临时账号和痕迹已清理。
- 冻结库及恢复目录位于`/opt/business-analysis-backups/`，完成复核后accept-release状态为accepted。与本次无关的`market-analysis.service`既有failed状态未修改；详细证据与恢复点见`docs/RELEASE_REVIEW_v1.0.148.md`。

## 2026-09-03 v1.0.147 日常上传修复与四份清单导入

- 用户反馈数据上传有问题。真实页面四份清单预览长期等待，最终经代表因末尾“合计”行无有效年月被拒绝；同时定位大库按月、按缺失字段重复扫描导致的预览性能问题。
- 合并PR #25，应用提交 `2c78e3bec71b7b17ed8bbdffcebf3acb40eb5c57`。期间筛选和旧字段保护改为批量检查，末尾明确总计行单独提示并忽略；前端增加预览取消、全响应超时、迟到响应隔离和写入结果待核实保护，避免重复导入。业务聚合公式与历史客户匹配规则不变。
- 最终Linux回归731项通过、依赖扫描无已知漏洞，GitHub验证与GHCR发布成功。生产副本上同一批文件预览47.25秒、导入172.71秒通过；2026年审计零问题，2025年原有0.01万元机构日汇总差异前后一致。
- 可信归档保库部署Ubuntu，复用依赖、跳过无新增迁移的历史重建；生产预览46.08秒、导入181.88秒，四份全部success，新增记录291—294，错误0。运行账号及生产导入锁/事务保持生效。
- 浏览器自动选文件返回Not allowed，本次通过与Web共用的服务器服务完成正式导入；已登录看板显示更新结果，正式公网v1.0.147及脚本一致、16MiB匿名请求返回401而非413。网页文件选择至提交的完整自动验收仍受工具能力限制，不能表述为已验证。
- 最终生产完整性/快速检查ok，2026年审计零问题；18张汇总表业务字段及客户事实与候选一致，汇总表运行时间戳差异已逐列确认。账号、权限、目标和未覆盖的旧年度主汇总与冻结库一致，服务及页面无新增异常，未回退。
- 完整证据、最终复核状态与冻结恢复点见 `docs/RELEASE_REVIEW_v1.0.147.md`。上线后的新导入必须纳入后续恢复判断，不能直接覆盖为发布前数据库。

## 2026-09-02 v1.0.146 稳定性修复、发布与完整复核

- PR #24 已合并，应用提交 `173ed6f362178becfbfdbaed15d90d932e4e72d6`。最终 Linux 回归654项通过，依赖扫描无已知漏洞，GitHub验证与GHCR镜像发布成功。
- 完成完整月/补充模式与预览确认、客户事实事务刷新、8张KPI共同指标合同、只读审计失败退出、镜像白名单及停服冻结恢复链。仅两条客户迁移时精准重建客户事实，不重复重建无关主聚合。
- 可信归档已部署Ubuntu，保留既有生产库；正式NPM公网HTTPS与本机版本均为v1.0.146，页面和4个更新脚本哈希一致。鉴权、敏感路径、实际页面交互、应用/数据权限和日志均通过。
- 停服冻结备份与上线库的18张主聚合表、目标、账号和权限逐行哈希一致；源表行数与原始保费不变。161组客户事实与源数据金额差额为零，2026年只读审计零问题，全库完整性检查ok。
- 独立Windows介质副本已还原并通过哈希及完整性检验；完整复核后接受发布并保留本次冻结库、恢复包和旧依赖。没有触发版本回退。
- 历史8组编号歧义按原号精确关联，未确认部分单列并提示，待权威映射补齐；不推定客户归属。凭据轮换没有平台证明，仍保留待办。
- 原工作区既有文档内容已纳入，另存受保护副本与stash。历史Git pack损坏已无损恢复并通过完整仓库检查，既有同步冲突与悬空对象均未删除。后续开发继续使用同步目录外的独立工作区。
- 详细验收、CI编号、镜像摘要和恢复位置见 `docs/RELEASE_REVIEW_v1.0.146.md`。本节优先于下方历史状态。
