...
- 新增交互式`configure-zhihu-api.sh`，密钥静默写入`/etc/business-analysis-market/market-analysis.env`，API验证失败自动恢复原配置；真实密钥不进入命令参数、Git或日志。
- 专项49项测试通过；全量383项中380项通过，3项产品结构测试因隔离工作树本地运行库缺少转型业绩基线数据失败，与本次市场研判改动无关。Python编译、`git diff --check`和两项部署脚本远程`bash -n`通过。知乎真实鉴权和生产部署待受保护配置完成后验收。
