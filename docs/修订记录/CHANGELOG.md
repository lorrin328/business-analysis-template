...
## [2026-09-07] fc8d8d9 fix: v1.0.151 长险活动率归零

修复混合历史月份与完整时间戳被统一格式推断漏算，以及整数等值浮点年月解析问题。新增强制聚合迁移，在现有生产明细上重建汇总，9月活动率恢复为19.6%（79/403.5）。827项Linux回归通过，GitHub PR #28合并、镜像发布及Ubuntu部署完成；公网业务、63个渠道月份对账与独立备份完整性检查通过。详见[发布验收](../RELEASE_REVIEW_v1.0.151.md)。

## [2026-08-31] feat: v1.0.144 机构连续挂零

按期交保费正向承保及业绩归属日计算机构、项目自然日连续挂零，位于机构维度保障类产品之后。新增独立活动汇总及强制迁移，保留既有净保费口径，显式处理缺日、断月和未开展状态。详见[需求报告](../需求/2026-08-31_机构维度连续挂零指标需求报告.md)。

## [2026-05-23] fix: v1.0.34 经代与转型截止日差异口径
**类型**：fix / data-quality

**变更内容**：
| 文档 | 变更 |
|------|------|
| [主 README](../../README.md) | 当前版本更新为 v1.0.34 |
| [CHANGELOG](../../CHANGELOG.md) | 同步 v1.0.34 发布说明 |

**代码修复**：
1. 导入层允许转型与经代日级截止日不同，保留源数据真实截止日并返回提示。
2. 趋势年度/季度/月度混合统计按共同截止日截断。
3. 产品结构在经代与转型混合展示时按共同截止日过滤。
4. 补充轻量测试环境与完整 FastAPI 测试环境的回归覆盖。

## [2026-05-22] fix: v0.9.997 参数设置口径、Token 与部署同步
**类型**：fix / safety / deployment

**变更内容**：
| 文档 | 变更 |
|------|------|
| [主 README](../../README.md) | 当前版本更新为 v0.9.997 |
| [CHANGELOG](../../CHANGELOG.md) | 同步 v0.9.997 发布说明 |

**代码与部署修复**：1. 统一前端、Docker Compose 和部署脚本的后台管理 Token 配置方式，取消默认明文值。2. 修复 `product_config` 自动提取产品时对 `202605` 等年月格式识别失败的问题。3. 产品配置保存改为 upsert 并返回真实更新数量。4. KPI 接口新增 `protection_total`，保障类产品卡片按参数设置与目标展示。5. 参数设置弹窗对产品字段进行 HTML 转义。
---
