...
**代码与部署修复**：1. 统一前端默认管理 Token、Docker Compose 默认值和部署脚本 `.admin_env` 为 `Aaaaa8888%`。2. 修复 `product_config` 自动提取产品时对 `202605` 等年月格式识别失败的问题。3. 产品配置保存改为 upsert 并返回真实更新数量。4. KPI 接口新增 `protection_total`，保障类产品卡片按参数设置与目标展示。5. 参数设置弹窗对产品字段进行 HTML 转义。
---
