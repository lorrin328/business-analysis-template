...
新研判在原宏观、监管、同业、条线四层中增加产品专题筛选，产品模块仍属于同业层，不改变旧报告结构。来源侦察预留至少三个产品搜索主题，覆盖分红、养老年金及公司条款/上市撤销资料；主研和修复共用产品研究合同。

生产安装时启用 `MARKET_ANALYSIS_REQUIRE_PRODUCT_RESEARCH=1`。新报告必须包含 `productResearch`，列出覆盖状态、产品模块、实际搜索主题和证据缺口。有证据时生成产品模块；无证据时明确缺少哪些资料及下一步，禁止填造产品。旧报告读取不要求补写字段。
