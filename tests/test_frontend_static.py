...
def test_frontend_centralizes_read_api_fetches():
    ...
    assert '<script src="js/month-multi-select.js?v=1.0.115"></script>' in html
    assert '<script src="js/api-client.js?v=1.0.115"></script>' in html
    assert '<script src="js/auth-ui.js?v=1.0.115"></script>' in html
    assert '<script src="js/export-excel.js?v=1.0.115"></script>' in html
    assert '<script src="js/upload.js?v=1.0.115"></script>' in html
    assert '<script src="js/target-modal.js?v=1.0.115"></script>' in html
    assert '<script src="js/kpi-cards.js?v=1.0.115"></script>' in html
    assert '<script src="js/platform-trend.js?v=1.0.115"></script>' in html
    assert '<script src="js/team-analysis.js?v=1.0.115"></script>' in html
...
def test_dashboard_config_is_loaded_before_kpi_cards():
    ...
    assert '<script src="js/dashboard-config.js?v=1.0.115"></script>' in html
...
def test_product_config_modal_is_outside_html_shell():
    ...
    assert '<script src="js/product-config-modal.js?v=1.0.115"></script>' in html
...
def test_dashboard_toolbar_actions_are_bound_by_runtime_module():
    ...
    assert '<script src="js/dashboard-actions.js?v=1.0.115"></script>' in html
...
def test_personnel_management_page_is_admin_only_calculator_runtime():
    ...
    assert 'data-permission="personnel_management"' in html
    assert '<script src="/js/personnel-management.js?v=1.0.111"></script>' in page
...
def test_honor_page_is_separate_runtime():
    ...
    assert 'data-tab="people">人员追踪</button>' in honor_html
    assert 'data-tab="data">数据与规则</button>' in honor_html
    assert "数据适配检查" not in honor_html
    assert "总览驾驶舱" not in honor_html
    assert 'id="batchSummary"' in honor_html
    assert 'id="auditResult"' in honor_html
    ...
    assert "function renderAnalysis()" in honor_js
    assert "function renderPeople()" in honor_js
...
def test_kpi_modal_content_is_outside_html_shell():
    ...
    assert '<script src="js/kpi-modal-content.js?v=1.0.115"></script>' in html
...
def test_org_analysis_has_expand_mode_and_colored_indicators():
    ...
    assert 'src="js/org-analysis.js?v=1.0.115"' in html
...
def test_kpi_cards_js_is_runtime_owner_for_kpi_cards():
    ...
    assert 'src="js/kpi-cards.js?v=1.0.115"' in html
...
def test_kpi_insight_panel_is_removed():
    html = read_html()
    kpi = read_js("kpi-cards.js")

    assert 'id="kpiInsightPanel"' not in html
    assert ".kpi-insight-panel" not in html
    assert "kpi-insight-label" not in html
    assert "function renderKpiInsight" not in kpi
    assert "renderKpiInsight({" not in kpi
...
def test_structure_modules_are_same_level_with_tables():
    ...
    assert "function renderPayPeriodAverageTable()" in payperiod
...
def test_team_enhanced_panel_is_added_without_replacing_team_trend():
    ...
    assert "teamHighProductivityBusinessLineTable" in team
    assert "teamHighProductivityOrgLineTable" in team
    assert "累计期交保费60万元及以上人力分档" in team
    assert "仅展示OTO、证保" in team
    assert "highProductivity.definitions?.bands" in team
    assert "renderHighProductivityRows" in team
    assert "#teamHighProductivityBusinessLineTable" in html
    assert "#teamHighProductivityOrgLineTable" in html
    assert "#teamEnhancedPanel { min-width: 0; max-width: 100%; overflow: hidden; }" in html
    assert "#teamEnhancedPanel .structure-table-wrapper {" in html
...
def test_platform_trend_main_is_loaded_at_runtime_boundary():
    ...
    assert '<script src="js/platform-trend-main.js?v=1.0.115"></script>' in html
...
def test_unverified_targets_do_not_render_as_formal_achievement_judgement():
    ...
    assert '按日或自定义日期范围没有可直接对应的正式目标' in kpi
